...
**Update this AGENTS.md as part of the change, not after.** When a turn adds a feature, changes an
architectural rule, or touches deploy/infra setup, the corresponding section here gets edited in
the same turn — not left for later, not left for the user to ask for. A stale AGENTS.md is worse
than no AGENTS.md, because it's actively misleading rather than obviously absent. If you're not
...
app/
  ...
  _lib/
    ...
    navigation.js    pure contextual-return parsing shared by the SPA and recap route
...
Next's real server/client split is in play. This is exactly why AGENTS.md already says to verify UI
...
- Navigation uses visible `.nav-action` controls (icon plus plain-language label), and every saved
  game row includes a directional marker. Record links carry their origin in the query string so
  the recap can return to the leaderboard, player history, or stats page that opened it.
- The game picker is a three-box shelf on wide screens, two columns on compact tablets, and three
  horizontal box-art rows at phone widths. The mobile layout is intentionally compact enough to
  keep the whole choice set near one viewport; do not restore full-width 4:3 cards there.
...
Prefer the Browser pane (`.Codex/launch.json`, config `harmonies-counter`) over Bash for this.

**Codex:** use the built-in in-app Browser for local UI checks. Do not fall back to the optional
`agent-browser` shell command just because it appears in a verification skill; it is not installed
for this repo and duplicates the Browser capability already available in Codex.
