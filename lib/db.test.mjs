...
// Accepting these at the normalizeEndedBy level is safe now that a winnerSeat exists in the
// save-game payload to say who won (app/api/save-game/route.js's validate()) — normalizeEndedBy
// itself doesn't know about winnerSeat, that's the route's job, but ENDED_BY and ACCEPTED_ENDED_BY
// must agree once the plumbing is in place, or this test (parametrized over ACCEPTED_ENDED_BY
// above) silently stops covering them.
test('normalizeEndedBy: accepts both supremacy endings now that a winnerSeat can describe who won', () => {
  assert.equal(normalizeEndedBy('military_supremacy'), 'military_supremacy');
  assert.equal(normalizeEndedBy('scientific_supremacy'), 'scientific_supremacy');
  assert.deepEqual(ENDED_BY, ACCEPTED_ENDED_BY,
    'every ended_by the column supports should now be accepted by the API');
});
