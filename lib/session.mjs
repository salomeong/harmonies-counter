...
export async function getSessionByPublicId(publicId) {
  ...
  const photos = await sql`
    SELECT id, blob_url AS "blobUrl", created_at AS "createdAt"
    FROM session_photos
    WHERE session_id = ${session.id}
...
export async function recordSessionPhoto({ sessionId, blobUrl }) {
  const sql = getSql();
  await sql`INSERT INTO session_photos (session_id, blob_url) VALUES (${sessionId}, ${blobUrl})`;
}
