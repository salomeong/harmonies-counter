...
// makeScorer(harmonies, …).breakdown()/.total() and prints the results as stable JSON. `--check`
// compares that output against the checked-in scripts/score-fixtures.expected.json (generated
// from the CURRENT behaviour, before any later refactor) and exits non-zero on any drift.
//
// This file used to call the bare breakdown()/totalPoints() functions that lived in
// src/scoring.js; those were deleted once index.html was rewired onto makeScorer() (see
// CLAUDE.md's scoring-model section), so this script now builds a scorer instead. The expected
// JSON is intentionally NOT regenerated by that change — scorer.breakdown() returns its keys in a
// different order (`total` first, then the `sums` groups) than the old breakdown() did
// (`landscape, animals, spirit, total`), so each fixture's breakdown is re-assembled into the
// original key order below. If the check ever fails, the *numbers* changed, not just their order.
//
// Usage:
...
function computeFixtures(){
  return fixtures.map(f => {
    const scorer = makeScorer(harmonies, () => f.variant);
    const b = scorer.breakdown(f.player);
    return {
      name: f.name,
      variant: f.variant,
      // Re-assembled in the pre-refactor key order (landscape, animals, spirit, total) — see the
      // header comment. scorer.breakdown(p) is still the sole source of the numbers.
      breakdown: { landscape: b.landscape, animals: b.animals, spirit: b.spirit, total: b.total },
      totalPoints: scorer.total(f.player)
    };
  });
}
